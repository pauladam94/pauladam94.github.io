...
#block(
  ...
  grid(columns: (auto, 1fr), column-gutter: 1em,
    block(radius: 5%, box(stroke : 1pt + rgb(0, 0, 0), radius: 3pt, clip: true)[#image(gen.picture, width: 130pt)]),
...
#let stage(values) = {
  ...
  for (title, value) in values [
    ...
    #grid(columns: col_setup, stroke: debug,
      align(left, stack(spacing: 0.6em,
        text(0.8em, value.place),
        text(0.8em, value.affiliation)
      )),
      align(center + horizon, stack(spacing: 0.5em,
        underline[*#get-txt(value.title)*],
        [#get-txt(value.subtitle)]
      )),
      align(right, 
        grid(columns:(1fr, 1fr), 
          text(0.8em, get-txt(value.duree)),
          text(0.8em, get-txt(value.time))
        )
      )
    )
    
    #set par(justify: true)
    #box(inset: (left: 1em, right: 1em), stroke: 0pt + green)[#h(1em) #get-txt(value.text)]
...
#let etude(values) = {
  for (title, value) in values [
    ...
    #grid(columns: col_setup, stroke: debug,
      ...
      align(right, stack(dir: ltr, spacing: 2em,
...
#for (part, values) in data {
  ...
  align(center, box(
    stroke: 0pt,
    width: 110%,
    grid(columns: 2, column-gutter: 0.3em,
      [*#get-txt(values.title)*],
      align(center + horizon, box(height: 1pt, radius: 3pt, width: 100%, fill: black))
    )
  ))
