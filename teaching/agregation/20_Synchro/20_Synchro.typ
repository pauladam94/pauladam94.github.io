...
#item("Définition")[Un Fil d'exécution][  
  est une partie de programme en exécution avec ses données. Il comprend donc la pile et le compteur d'instruction. Les données et le code étant partagées entre tous les fils d'exécutions d'un même programme.
]


#item("Définition")[Des ressources dites partagées][
  sont communes entres les fils. C'est sur ces données que des stratégies de synchronisation devront être appliqués.
]

#item("Remarque")[Ordonnancement.][
  De nombreux fils d'éxécutions s'éxécutent en parallèle sur un ordinateur. Le système d'exploitation choisit à chaque instant quels fils a accès au processeur pour s'éxécuter.
]

#dev[
  #item("Exemple")[Le Tri Fusion][
    est un algorithme de tri parallélisable. La stratégie de synchronisation consiste ici à lancer des fils sur des parties distinctes du tableau.
  ]
]
...
#item("Propriété de sûreté")[L'Exclusion-mutuelle][
énonce une propriété de sûreté, pour une section de code fixée, qu'au plus un fil d'exécution ne peut avoir acquit la section à la fois. 
]
...
#item("Remarque")[L'attente active][
  consiste à attendre. On l'écrit par une boucle `while` sur la condition voulue, comme fait par l'algorithme de Peterson.
]
]
...
// Autre dev possible: compteur approximatif multithreadé (dans l'OSTEP)

// Autres sources possibles : CSAPP, OSTEP
